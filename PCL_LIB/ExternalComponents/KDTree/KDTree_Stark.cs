...
namespace PCLLib
{
    public class KDTree_Stark
    {
        ...
        public static List<KeyValuePair<int, double>>  LatestSearchResults;
        public Vertex pivot;
        ...
        public void ResetSearch()
        {
            ...
            LatestSearchResults = new List<KeyValuePair<int, double>>();
        }
        ...
        //}
        //	Make a new tree from a list of points.
        public static KDTree_Stark Build(List<Vertex> points)
        {
            try
            {
        ...
        public int FindNearest(Vertex pt)
        {
            ...
            //LatestSearchResults.Sort(new KeyValueComparer());
            return bestIndex;
        }
        //	Find the nearest point in the set to the supplied point.
        public int FindNearest_ExcludeTakenPoints(Vertex pt)
        {
            double bestSqDist = 1000000000D;
            int bestIndex = -1;
        ...
        static KDTree_Stark MakeFromVectorsRecursive(int depth, int stIndex, int enIndex, List<Vertex> points,int[] inds)
        {
        ...
        static int FindSplitPoint(List<Vertex> points, int[] inds, int stIndex, int enIndex, int axis)
        {
            double a = points[inds[stIndex]].Vector[axis];
            double b = points[inds[enIndex]].Vector[axis];
            int midIndex = (stIndex + enIndex) / 2;
            double m = points[inds[midIndex]].Vector[axis];
        ...
        //	of its plane.
        public static int FindPivotIndex(List<Vertex> points, int[] inds, int stIndex, int enIndex, int axis)
        {
        ...
        void SearchRecursive(Vertex pt, ref double bestSqSoFar, ref int bestIndex)
        {
            double mySqDist = Distance_Euclid(pivot, pt);

            LatestSearchResults.Add(new KeyValuePair<int, double>(pivotIndex, mySqDist));
            ...
            double planeDist = pt.Vector[axis] - pivot.Vector[axis]; //DistFromSplitPlane(pt, pivot, axis);
        ...
        //	Recursively search the tree.
        void SearchRecursive_ExcludeTakenPoints(Vertex pt, ref double bestSqSoFar, ref int bestIndex)
        {
            double mySqDist = Distance_Euclid(pivot, pt);
        ...
        double DistFromSplitPlane(Vertex pt, Vertex planePt, int axis)
        {
            return pt.Vector[axis] - planePt.Vector[axis];
        }
        ...
        public double Distance_Euclid(double[] p1, double[] p2)
        {
            double fSum = 0;
            ...
            return System.Math.Sqrt(fSum);
        }
        public double Distance_Euclid(Vertex p1, Vertex p2)
        {
            double fSum = 0;
            double fx = (p1.Vector.X - p2.Vector.X);
            double fy = (p1.Vector.Y - p2.Vector.Y);
            double fz = (p1.Vector.Z - p2.Vector.Z);
