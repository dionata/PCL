...
-------------------------------------------------------------------------
This program will generatePart a given number of points uniformly distributed
on the surface of a sphere. The number of points is given on the command
line as the first parameter.  Thus `sphere 100' will generatePart 100 points 
on the surface of a sphere, and output them to stdout.
	A number of different command-line flags are provided to set the 
radius of the sphere, control the output format, or generatePart points on 
...
are uniformly distributed on the sphere and that is scaled to an ellipsoid.
	Random.nextDouble() is used to generatePart random numbers, automatically 
