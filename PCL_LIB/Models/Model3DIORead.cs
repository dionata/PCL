...
namespace PCLLib
{
    ...
    {
        private static double[,] matrixEspuma = new double[10,3]; //matrix x pontosXYZ
        private static int pointExt = 0;
        private static int[] trianguloInt = new int[3];
        private static double[,] InfoPosVarredura = new double[10, 3]; //matrix x pontosXYZ
        ...
        private static void readOBJfile(string fileOBJ, Model3D myNewModel)
        {
            ...
            try
            {
                ...
                using (StreamReader streamReader = new StreamReader(fileOBJ))
                {
                    ...
                    Vector3d vectorAux = new Vector3d(-350,800,-200);                   
                        ...
                        if (strArrayRead.Length >= 0)
                        {
                            switch (strArrayRead[0].ToLower())
                            {
                                case "mtllib":
                                    ...
                                    {
                                        //System.Windows.Forms.MessageBox.Show("Error reading obj file (mtllib) in line : " + line);//dio
                                    }

                                    //myNewModel.GetTexture(strArrayRead[1], fileOBJ);//dio
                                    break;
                                case "v"://Vertex
        ...
        private static Triangle helper_ReadTriangle(string[] strArrayRead, Model3D myNewModel)
        {
            ...
            if(strArrayRead[1] == "2366//2366")
            {
                strArrayRead[1] = strArrayRead[1];
            }
                ...
                foreach (string str2 in strArrayRead)
                {
                    if (str2.ToLower() != "f")
                    {
                        try
                        {
                            ...
                            //TODO: adicionar o deslocamento dos pontos adicionados 
                            result = result + pointExt;
        ...
        /// 
        /// </summary>
        /// <param name="strArrayAdd"></param>
        /// <param name="myNewModel"></param>
        /// <returns></returns>
        public static Triangle helper_AddTriangle(Model3D myNewModel, int a_, int b_, int c_)
        {
            ...
            try
            {
                ...
                try
                {
                    for(int i=0;i< 3; i++)
                    { 
                        a.IndVertices.Add(trianguloInt[i] - 1); //-1
                        int indVertex = a.IndVertices.Count - 1;//3-1=2
                        int indPart = myNewModel.Parts.Count - 1;//
                        ...
                        myNewModel.vertexList[a.IndVertices[indVertex]].IndPart.Add(indPart);
                            ...
                            /*
                            int index2 = addPointsExta - 1;

                                a.IndNormals.Add(index2);
                                int num2 = addPointsExta - 1;


                                myNewModel.vertexList[a.IndVertices[indVertex]].IndexNormals.Add(num2);


                                int num2_ = addPointsExta - 1;


                                a.IndTextures.Add(num2_);
                         */
                    }
                }
                catch (Exception err1)
                ...
                using (StreamReader streamReader = new StreamReader(str + TexFile))
                {
                    while (!streamReader.EndOfStream)
                    {
                        ...
                        if (strArray2[0].ToLower() == "map_kd")
                            this.TextureBitmap = new Bitmap(str + strArray2[1].Replace(",", "."));
                    }
                    streamReader.Close();
                }
            }
