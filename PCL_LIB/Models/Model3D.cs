...
namespace PCLLib
{
    public partial class Model3D
    {
        ...
        public static Vector3d vetTransl_port = new Vector3d(0, 0, 0);
        public static Vector3d refTranslMarcacaoCorte_port = new Vector3d(0, 0, 0);
        public static Vector3d refTranslApagador_port = new Vector3d(0, 0, 0);
        public static Vector3d refRotlApagador_port = new Vector3d(0, 0, 0);
        public static Vector3d vetRot_port = new Vector3d(0, 0, 0);
        public static List<Vector3d> vetRot_portDesfazer = new List<Vector3d>();
        public static Vector3d refRotMarcacaoCorte_port = new Vector3d(0, 0, 0);
        ...
        public Model3D(string file)
        {
            ...
            else if (str == ".lab3d")
            {
                try
                {
                    ...
                    Bitmap PictureTexture = (Bitmap)null;
                    if ((int)dataRow["intTexType"] == 2)
                    {
                        try
                        {
                            PictureTexture = new Bitmap(Path.GetDirectoryName(file) + "\\" + Path.GetFileNameWithoutExtension(file) + ".JPG");
                        }
                        catch
                        {
                ...
                {
                    throw new Exception("Invalid PCLLib file");
                }
            }
            if (!(this.Name == ""))
                return;
            ...
            this.Name = Path.GetFileName(file); // .GetFileNameWithoutExtension(file);
        }
        ...
        public void Create3DModel(string Name, Model3D.CoordFuncXYZ F, float umin, float umax, float vmin, float vmax, int uPts, int vPts, Vector3d cor)
        {
            ...
            this.Create3DModel(Name, VertENormal, textureCoords, (float[,][])null, cor, true);
        }
        ...
        public void Render(bool drawTransparent, string nomeModelo)
        {
            if (nomeModelo == "Marcação Corte")
            {
                refTranslMarcacaoCorte_port.X = vetTransl.X;
                refTranslMarcacaoCorte_port.Y = vetTransl.Y;
                refTranslMarcacaoCorte_port.Z = vetTransl.Z;

                refRotMarcacaoCorte_port.X = vetRot.X;
                refRotMarcacaoCorte_port.Y = vetRot.Y;
                refRotMarcacaoCorte_port.Z = vetRot.Z;
            }
            else
            {
               // if (GLSettings.teste)
                //{
                    vetTransl_port.X = vetTransl.X;
                    vetTransl_port.Y = vetTransl.Y;
                    vetTransl_port.Z = vetTransl.Z;          
                    vetRot_port.X = vetRot.X;
                    vetRot_port.Y = vetRot.Y;
                    vetRot_port.Z = vetRot.Z;
                    GLSettings.teste = false;
               // }
            }
        ...
        private void ApplyTexture()
        {
            try
            {
                if (this.TextureBitmap != null)
                {
                    if (this.GLTexture <= 0)
                    {
                        Bitmap bitmap = ResizeToPowerOf2((Image)new Bitmap((Image)this.TextureBitmap));
        ...
        private static Bitmap ResizeToPowerOf2(Image img)
        {
        ...
        public void SelectPart(int indPart)
        {
            ...
            this.Render((double)this.Parts[indPart].Transparency != 0.0, null);
        }
        ...
        public void CreateFromOpenTKLibTbl(DataTable OpenTKLibTbl, Bitmap PictureTexture, out List<string> BuildLogs)
        {
            ...
            try
            {
                ...
                this.ModelColor =Color.FromArgb((int)dataRow["intColorR"], (int)dataRow["intColorG"], (int)dataRow["intColorB"]);
            ...
            {
                throw new Exception("Invalid PCLLib format");
            }
